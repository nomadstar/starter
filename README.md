...
3. **Control Remoto Vía Telegram:** Monitorea y dirige todo el escuadrón desde tu teléfono móvil. El sistema envía logs en vivo por Telegram y hace *Long Polling*. Puedes aprobar planes escribiendo `/approve` en Telegram, descargar archivos generados directamente al chat con `/cat <ruta>` o `/get <ruta>`, o detener una IA rebelde escribiendo `/kill` 😈. Usa el comando de Neovim `:NewTeleBot` para cargar automáticamente todos estos atajos (shortcuts) en el menú rápido de tu bot.
...
6. **Autoevaluación (Defiende tu Código), Cooperación y Triage JSON:** La cadena de código generada se envía al Arquitecto de la Nube para defender su trabajo. El Arquitecto no solo evalúa el código final (Score 0-100), sino que devuelve métricas de **Cooperación** y puntajes individuales para cada modelo participante.
   ...
   - **Score < 80:** Error grave. Se fuerza a Ollama a reescribir. **Liderazgo Dinámico y Mentoría:** El modelo con peor rendimiento intenta redimirse como *Aprendiz* (recibiendo un `mentorship_advice` en su prompt para mejorar), mientras que el mejor modelo asume como su *Maestro* revisor. Esto evita la convergencia del enjambre a un solo modelo dominante. Además, el algoritmo *Short-Circuit* silenciará silenciosa y automáticamente cualquier alucinación si un modelo enloquece o arroja Markdown sin comandos válidos, cortando el problema de raíz sin lanzar falsos errores de red.
...
10. **Memoria Persistente y Aislamiento:** El Arquitecto posee una memoria a corto y largo plazo inyectada automáticamente (`.ai_router_state.md`) que le recuerda la directiva inicial y qué archivos ya fueron construidos. ¡El Arquitecto nunca olvida su propósito original sin importar cuántas horas lleve trabajando!
...
13. **MidnightMonster Mode (Delegación Nocturna Autónoma):** Entre las 10:00 PM y las 8:00 AM, si activas `MIDNIGHT_MONSTER="true"` en tu `.env`, el sistema dejará de pedirte permisos humanos para avanzar. En su lugar, el Arquitecto Cloud evaluará el código de los Ollamas locales o sus sugerencias de nuevos archivos para determinar si cumplen la meta. Si los aprueba, el sistema avanzará solo. ¡El humano puede irse a dormir mientras el motor local y la validación en la nube trabajan juntos toda la noche! Los comandos `/kill` y `/q` de Telegram siguen funcionando si necesitas intervenir.
